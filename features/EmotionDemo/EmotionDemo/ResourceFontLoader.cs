...
namespace EmotionDemo
{
    ...
    public class ResourceFontLoader : CallbackBase, FontCollectionLoader, FontFileLoader
    {
        ...
        //https://stackoverflow.com/questions/40967800/enumerate-supported-characters-glyphs-in-a-specific-font-with-uwp-c
        public ResourceFontLoader(Factory factory, Stream fontfile)
        {
            _factory = factory;
            ...
            try
            {
                using (fontfile)
                {
                    var fontBytes = Utilities.ReadStream(fontfile);
                    var stream = new DataStream(fontBytes.Length, true, true);
                    stream.Write(fontBytes, 0, fontBytes.Length);
                    stream.Position = 0;
                    _fontStreams.Add(new ResourceFontFileStream(stream));
